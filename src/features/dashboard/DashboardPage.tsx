...
export function DashboardPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 xl:grid-cols-2"><Card><h3 className="mb-4 font-black">Pending Approvals</h3>{[
      ["Leave approvals", pending.leaves],
      ["Attendance regularizations", pending.regularizations],
      ["Timesheet approvals", pending.timesheets],
      ["Invoice actions", pending.invoices]
    ].map(([label,value])=><div key={label} className="mb-3 flex items-center justify-between rounded-xl bg-slate-50 p-3"><span>{label}</span><span className="font-black">{value}</span></div>)}</Card><Card><h3 className="mb-4 font-black">At-Risk Alerts</h3>{s.projects.filter(p=>p.health!=="Green").map(p=><div key={p.id} className="mb-3 flex items-center justify-between rounded-xl bg-slate-50 p-3"><span>{p.name}</span><StatusBadge value={p.health}/></div>)}</Card></div>
  </div>;
}
