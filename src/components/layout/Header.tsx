...
export function Header({ onMenu }: { onMenu:()=>void }) {
  const role = useAppStore(s=>s.role), setRole = useAppStore(s=>s.setRole);
  return <header className="sticky top-0 z-30 border-b border-black/5 bg-[#f5f2ea]/90 px-4 py-3 backdrop-blur lg:px-6">
    <div className="flex items-center gap-3">
      ...
      <div className="ml-auto flex items-center gap-2">
        <select value={role} onChange={e=>setRole(e.target.value)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold">
          {["Super Admin","HR Manager","Project Manager","Mentor","Finance Manager","Employee","Intern","Student","Corporate Partner"].map(r=><option key={r}>{r}</option>)}
        </select>
        ...
        <button className="flex items-center gap-2 rounded-xl border bg-white px-3 py-2 text-sm font-bold"><UserRound size={18}/> Admin</button>
      </div>
    </div>
  </header>;
}
